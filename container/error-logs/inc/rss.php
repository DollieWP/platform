...
switch ( $format )
{
	...
	case 'RSS':
		...
		define( 'TIME_ZONE' , $tz );
		define( 'FEEDCREATOR_VERSION' , 'Log Viewer v' . get_current_pml_version() );
		$rss                              = new UniversalFeedCreator();
		$rss->title                       = sprintf( __t( "Log Viewer : %s" ) , $files[ $file_id ][ 'display' ] );
		$rss->description                 = ( empty( $search ) )
			? sprintf( __t( "Pimp logs for file %s" ) , $files[ $file_id ][ 'path' ] )
			: sprintf( __t( "Pimp logs for file %s with search %s" ) , $files[ $file_id ][ 'path' ] , $search );
		...
		$image->link                      = $link;
		$image->description               = __t( "Feed provided by Log Viewer" );
