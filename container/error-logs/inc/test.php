...
if ( @$_POST['action'] === 'regextest' )
{
	...
	if ( ! is_array( $match ) ) {
		...
		$return['msg'] = '<div class="alert alert-danger"><strong>' . __t('Error') . '</strong> '. __t('Match is not a valid associative array') . '</div>';
	...
	if ( ! is_array( $types ) ) {
		...
		$return['msg'] = '<div class="alert alert-danger"><strong>' . __t('Error') . '</strong> '. __t('Types is not a valid associative array') . '</div>';
		echo json_encode( $return );
		die();
	}
	...
	if ( @preg_match( $regex , 'this is just a test !' ) === false ) {
		...
		$return['msg'] = '<div class="alert alert-danger"><strong>' . __t('Error') . '</strong> '. __t('RegEx is not a valid PHP PCRE regular expression') . '</div>';
		echo json_encode( $return );
		die();
	}
...
//////////////////////
$lemma = array(
	"configuration_copied" => __t( "Configuration array has been copied to your clipboard!" ),
);


?><!DOCTYPE html><!--[if lt IE 7]><html class="no-js lt-ie9 lt-ie8 lt-ie7"><![endif]--><!--[if IE 7]><html class="no-js lt-ie9 lt-ie8"><![endif]--><!--[if IE 8]><html class="no-js lt-ie9"><![endif]--><!--[if gt IE 8]><!--><html class="no-js"><!--<![endif]--><head><meta charset="utf-8"><meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"><meta name="description" content=""><meta name="viewport" content="width=device-width"><title><?php echo TITLE;?></title><?php $fav = '../' ; include_once 'favicon.inc.php'; ?><link rel="stylesheet" href="../css/pml.min.css"><script>var lemma = <?php echo json_encode($lemma);?>;</script></head><body><!--[if lt IE 7]><p class="chromeframe">You are using an <strong>outdated</strong> browser. Please <a href="http://browsehappy.com/">upgrade your browser</a> or <a href="http://www.google.com/chromeframe/?redirect=true">activate Google Chrome Frame</a> to improve your experience.</p><![endif]--><div class="navbar navbar-inverse navbar-fixed-top"><div class="container"><div class="logo"></div><div class="navbar-header"><a class="navbar-brand" href="?<?php echo $_SERVER['QUERY_STRING'];?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php _pml_e('Debugger');?></a></div></div></div><div class="container"><br><?php
if ( ( ! file_exists( $access_file ) ) && ( ! Sentinel::isAdmin() ) ) {
	echo '<div class="row">';
	echo 	'<div class="col-xs-12"><div class="alert alert-danger">';
	echo 			__t( 'This page is protected for security reasons.');
	echo 		'</div>';
	if ( Sentinel::isAuthSet() ) {
		echo sprintf( __t('%sSign in%s as an administrator to view this page or follow instructions below.') , '<a href="../index.php?signin&attempt=' . urlencode( 'inc/test.php' ) . '">' , '</a>' ) . '<br/>';
	}
	echo 		'<br/>' . __t('To grant access, please create this temporary file on your server:');
	echo 		'<br/><br/>';
	echo 	'</div>';
	echo 	'<div class="col-md-8"><pre class="clipboard2content">touch \'' . dirname( __FILE__ ) . DIRECTORY_SEPARATOR . $access_file . '\'</pre></div>';
	echo 	'<div class="col-md-4"><a class="btn btn-primary clipboard" data-source=".clipboard2content" data-placement="right" data-text="' . h("Command has been copied to your clipboard!") . '">' . __t('Copy to clipboard') . '</a></div>';
	echo 	'<div class="col-xs-12">';
	echo 		'<br/>' . __t("Then reload this page.") . '<br/><br/>';
	echo 		'<button onclick="document.location.reload();" class="btn btn-primary">' . __t("Reload") . '</button>';
	echo 	'</div>';
	echo '</div>';
}
else {
?><ul class="nav nav-tabs" role="tablist"><li class="active"><a href="#quittab" role="tab" data-toggle="tab"><?php _pml_e('Quit');?></a></li><li><a href="#retestertab" role="tab" data-toggle="tab"><?php _pml_e('Regex tester');?></a></li><li><a href="#resamplestab" role="tab" data-toggle="tab"><?php _pml_e('Regex samples');?></a></li><li><a href="#configurationtab" role="tab" data-toggle="tab"><?php _pml_e('Configuration');?></a></li><li><a href="#passwordtab" role="tab" data-toggle="tab"><?php _pml_e('Password recovery');?></a></li><li><a href="#authactivation" role="tab" data-toggle="tab"><?php _pml_e('Authentication');?></a></li></ul><div class="tab-content"><div class="tab-pane active" id="quittab"><br><div class="row"><?php if ( ! Sentinel::isAdmin() ) : ?><div class="col-xs-12"><div class="alert alert-warning"><?php _pml_e('Please remove this temporary file on your server to disable the debugger!'); ?></div></div><br><br><div class="col-md-8"><pre class="clipboard3content"><?php echo 'rm \'' . dirname( __FILE__ ) . DIRECTORY_SEPARATOR . $access_file . '\''; ?></pre></div><?php echo '<div class="col-md-4"><a class="btn btn-primary clipboard" data-source=".clipboard3content" data-placement="right" data-text="' . h( "Command has been copied to your clipboard!" ) . '">' . __t('Copy to clipboard') . '</a></div>';?><?php else : ?><div class="col-xs-12"><div class="alert alert-info"><?php _pml_e('You can access the debugger because you are an administrator'); ?></div></div><?php endif; ?></div></div><div class="tab-pane" id="retestertab"><div class="panel-body"><form class="form-horizontal" role="form" id="regextest"><div class="form-group" id="GPinputLog"><label for="inputLog" class="col-sm-2 control-label"><?php _pml_e('Log');?></label><div class="col-sm-10"><textarea class="form-control test" id="inputLog" placeholder="Log"><?php
...
[27-11-2013:23:20:41 +0100] Single line is cool too';
								?></textarea></div></div><div class="form-group" id="GPinputRegEx"><label for="inputRegEx" class="col-sm-2 control-label"><?php _pml_e('RegEx');?></label><div class="col-sm-10"><textarea class="form-control test" id="inputRegEx" placeholder="RegEx"><?php
									echo '|^\[(.*)-(.*)-(.*):(.*):(.*):(.*) (.*)\] (.*)$|U';
								?></textarea></div></div><div class="form-group" id="GPinputMatch"><label for="inputMatch" class="col-sm-2 control-label"><?php _pml_e('Match');?><br><small><?php _pml_e('must be json encoded');?></small></label><div class="col-sm-10"><textarea class="form-control test" id="inputMatch" placeholder="Match" rows="5"><?php
								$match = array(
									'Date'  => array(
										'Y' => 3,
										'm' => 2,
								...
								?></textarea></div></div><div class="form-group" id="GPinputTypes"><label for="inputTypes" class="col-sm-2 control-label"><?php _pml_e('Types');?><br><small><?php _pml_e('must be json encoded');?></small></label><div class="col-sm-10"><textarea class="form-control test" id="inputTypes" placeholder="Types" rows="5"><?php
								$types = array(
									'Date'  => 'date:d/m/Y H:i:s /100',
									'Error' => 'txt',
								);
								echo json_indent( json_encode($types))
								?></textarea></div></div><div class="form-group" id="GPinputMultiline"><label for="inputMultiline" class="col-sm-2 control-label"><?php _pml_e('Multiline');?></label><div class="col-sm-10"><input class="form-control test" id="inputMultiline" placeholder="Multiline" value="Error"></div></div><div class="form-group"><div class="col-sm-offset-2 col-sm-10"><button type="submit" class="btn btn-success" title="<?php _h('Use CTRL-R shortcut instead of clicking on this button');?>" data-loading-text="<?php _h('Loading...');?>" id="regexTestertestBtn"><?php _pml_e('Test');?></button> &nbsp; <a class="btn btn-primary clipboard"><?php _pml_e('Copy to clipboard');?></a></div></div><div id="regexresult"></div></form></div></div><div class="tab-pane" id="resamplestab"><div class="panel-body"><?php
...
?></div></div><div class="tab-pane" id="configurationtab"><div class="panel-body"><div class="panel-group" id="accordion2"><div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title"><a data-toggle="collapse" data-parent="#accordion2" href="#collapseOne2">Code <code><?php echo get_config_file_path();?></code></a></h4></div><div id="collapseOne2" class="panel-collapse collapse"><div class="panel-body"><pre><?php if (file_exists( get_config_file_path() )) show_source( get_config_file_path() ); else echo 'configuration file does not exist'; ?></pre></div></div></div><div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title"><a data-toggle="collapse" data-parent="#accordion2" href="#collapseTwo2">Stats <code><?php echo get_config_file_path();?></code></a></h4></div><div id="collapseTwo2" class="panel-collapse collapse"><div class="panel-body"><pre><?php if (file_exists( get_config_file_path() ) ) var_export( @stat( get_config_file_path() ) ); else echo 'configuration file does not exist'; ?></pre></div></div></div><div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title"><a data-toggle="collapse" data-parent="#accordion2" href="#collapseFive2"><?php
											...
											if ( is_null( $current_user ) ) {
												_pml_e('Generated files with includes (no user logged in or no auth)');
											} else {
												echo sprintf( __t( 'Generated files with includes for user %s') , '<code>' . $current_user . '</code>' );
											}
									...
									?></pre></div></div></div><div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title"><a data-toggle="collapse" data-parent="#accordion2" href="#collapseFour2"><?php _pml_e('Rights');?></a></h4></div><div id="collapseFour2" class="panel-collapse collapse"><div class="panel-body"><pre><?php
									if (function_exists('posix_getpwuid')) {
										var_dump( @posix_getpwuid(posix_geteuid()) );
									} else {
										_pml_e('No POSIX functions...');
									}
									?></pre><?php
										$paths = array(
										...
										echo '<thead><tr><th>'.__t('Read').'</th><th>'.__t('Write').'</th><th>ID</th><th>'.__t('Path').'</th><th>'.__t('Real path').'</th></tr></thead>';
										echo '<tbody>';
										foreach ( $paths as $id => $file ) {
											...
											if ( empty( $rp ) ) {
												$rc = 'default';
												$wc = 'default';
												$rp = __t('Not allowed by open_basedir restriction');
											}
									...
									?></div></div></div><div class="panel panel-default"><div class="panel-heading"><h4 class="panel-title"><a data-toggle="collapse" data-parent="#accordion2" href="#collapseSix2">PHP Modules</a></h4></div><div id="collapseSix2" class="panel-collapse collapse"><div class="panel-body"><?php
										_pml_e('To be optimal, Log Viewer needs all these modules ok:');
										echo '<ul>';

										echo '<li>';
										if ( MB_SUPPORT === true ) {
											echo 'Multibyte String <span class="label label-success">' . __t('Yes') . '</span>';
										} else {
											echo 'Multibyte String <span class="label label-danger">' . __t('No') . '</span>';
											echo ' <span class="text-muted">(' . sprintf( __t('Follow instructions %shere%s to enable') , '<a href="http://php.net/manual/en/mbstring.installation.php" target="_blank">' , '</a>' ) . ')</span>';
										}
										echo '</li>';

										echo '<li>';
										if ( GETTEXT_SUPPORT === true ) {
											echo 'Gettext <span class="label label-success">' . __t('Yes') . '</span>';
										} else {
											echo 'Gettext <span class="label label-danger">' . __t('No') . '</span>';
											echo ' <span class="text-muted">(' . sprintf( __t('Follow instructions %shere%s to enable') , '<a href="http://php.net/manual/en/gettext.installation.php" target="_blank">' , '</a>' ) . ')</span>';
										}
										echo '</li>';
