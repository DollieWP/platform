...
function myErrorHandler( $errno, $errstr, $errfile, $errline ) {
	...
	case E_USER_WARNING:
		$return['singlewarning'] = sprintf( __t('<strong>PHP Warning</strong> [%s] %s') , $errno , $errstr );
		break;

	case E_USER_NOTICE:
		$return['singlenotice'] = sprintf( __t('<strong>PHP Notice</strong> [%s] %s') , $errno , $errstr );
		break;

	default:
		$return['singlewarning'] = sprintf( __t('<strong>PHP Unknown error</strong> [%s] %s') , $errno , $errstr );
		break;
	}
	return true;
}
...
function shutdown() {
	...
	if ( $error['type'] === E_ERROR ) {
		echo json_encode(
			array(
				'error' => sprintf( __t('<strong>PHP Error</strong> line %s: %s') , $error['line'] , $error['message'] )
			)
		);
	}
}
...
if 	( ! csrf_verify() ) {
	$return['error'] = __t( 'Please refresh the page.' );
...
switch ( @$_POST['action'] ) {
	...
	*/
	case 'change_password':
		...
		if ( ! Sentinel::isValidPassword( $username , $password1 ) ) {
			$errors[] = __t( 'Current password is not valid' );
			$fields[] = 'password1';
			$doit    = false;
		}
		if ( mb_strlen( $password2 ) < 6  ) {
			$errors[] = __t( 'Password must contain at least 6 chars' );
			$fields[] = 'password2';
			$doit    = false;
		}
		if ( $password2 !== $password3 ) {
			$errors[] = __t( 'Password confirmation is not the same' );
			$fields[] = 'password3';
			$doit    = false;
		}

		if ( $doit === true ) {
			if ( $_SERVER['SERVER_NAME'] === 'demo.logviewer.com' ) {
				$return['ok'] = __t( 'Password has been fakely changed on the demo!' );
			} else {
				$return['ok'] = __t( 'Password has been successfully changed!' );
	...
	*/
	case 'users_view':
		...
		if ( is_null( $user ) ) {
			$return['e'] = sprintf( __t('User %s does not exist') , '<code>' . $username . '</code>' );
		}
		else {
			unset( $user['pwd'] );
			if (isset($user[ 'lastlogin' ]['ts']))     $user[ 'lastlogin' ]['ts']     = date( 'Y/m/d H:i:s' , (int)$user[ 'lastlogin' ]['ts'] );
	...
	*/
	case 'users_add':
		...
		if ( empty( $username ) ) {
			$errors[ 'username' ] = __t( 'Username is required' );
		}
		else if ( ( $type === 'add' ) && ( Sentinel::userExists( $username ) ) ) {
			$errors[ 'username' ] = sprintf( __t('User %s already exists') , '<code>' . $username . '</code>' );
		}

		if ( ( ( $type === 'edit' ) && ( ! empty( $password ) ) ) || ( $type === 'add' ) ) {
			if ( mb_strlen( $password ) < 6 ) {
				$errors[ 'password' ] = __t( 'Password must contain at least 6 chars' );
			}
			if ( $password !== $password2 ) {
				$errors[ 'password2' ] = __t( 'Password confirmation is not the same' );
			}
		}
	...
	*/
	case 'users_delete':
		...
		if ( $username === $current_user ) {
			$return['error'] = __t('Please do not shoot yourself in the foot!');
		}
		else {
	...
	*/
	case 'anonymous_list':
		...
		if ( Sentinel::isAnonymousEnabled( $files ) ) {
			$r = '<div class="alert alert-info">' . __t('<strong>Anonymous access is enabled</strong>. Genuine users have to click on the user menu to sign in and access more logs.') . '</div>';
		} else {
			$r = '<div class="alert alert-info">' . __t('<strong>Anonymous access is disabled</strong>. All users have to sign in from the sign in screen.') . '</div>';
		}

		$r .= '<div class="form-group">';
		$r .= '	<label class="col-sm-4 control-label"></label>';
		$r .= '	<div class="col-sm-8">';
		$r .= __t("Select which log files can be viewed without being logged.");
		$r .= '		(<a href="#" class="logs-selector-toggler">'. __t('Toggle all log files') . '</a>)';
		$r .= '	</div>';
		$r .= '</div>';
		...
		foreach( $files as $file_id => $file ) {
			...
			if ( isset( $files[ $file_id ][ 'oid' ] ) ) {
				...
				if ( isset( $files[ $file_id ][ 'count' ] ) ) {
					...
					if ( $remain === 1 ) {
						$paths .= ' ' . __t( 'and an other file defined by glob pattern' );
					}
					else if ( $remain > 1 ) {
						$paths .= ' ' . sprintf( __t( 'and %s other possible files defined by glob pattern' ) , $remain );
					}
				}
				$color = 'warning';
			}
			...
			$r .= '				<input type="radio" name="f-' . $fid . '" id="anonymous-f-' . $fid . '-true" value="1"' . $ec . '/> '.__t('Yes');
			$r .= '			</label>';
			$r .= '			<label class="btn btn-xs logs-selector-no ' . $d . '">';
			$r .= '				<input type="radio" name="f-' . $fid . '" id="anonymous-f-' . $fid . '-false" value="0"' . $dc . '/> '.__t('No');
			$r .= '			</label>';
			$r .= '		</div>';
	...
	*/
	case 'profile_get':
		...
		$r .= '<div class="form-group">';
		$r .= 	'<label for="" class="col-sm-3 control-label">' . __t('Access token') . '</label>';
		...
		$r .=     '<a class="btn btn-xs btn-primary clipboardat">' . __t('Copy to clipboard') . '</a>';
		$r .= 	'</div>';
		$r .= '</div>';
		$r .= '<script>clipboard_enable( "a.clipboardat", "#prat" , "top" , "' . h( __t('Access token copied!') ) . '" );</script>';

		$r .= '<div class="form-group">';
		$r .= 	'<label for="" class="col-sm-3 control-label">' . __t('Presalt key') . '</label>';
		$r .= 	'<div class="col-sm-5">';
		$r .= 		'<div class="input-group">';
		...
		$r .= 	'</div>';
		$r .= 	'<div class="col-sm-4">';
		$r .=     '<a class="btn btn-xs btn-primary clipboardhp">' . __t('Copy to clipboard') . '</a>';
		$r .= 	'</div>';
		$r .= '</div>';
		$r .= '<script>clipboard_enable( "a.clipboardhp" , "#prhp" , "top" , "' . h( __t('Presalt key copied!') ) . '" );</script>';

		$r .= '<div class="form-group">';
		$r .= 	'<label for="" class="col-sm-3 control-label"></label>';
		$r .= 	'<div class="col-sm-9">';
        $r .=   '<input type="checkbox" name="regenerate" value="1"/> ' . __t('Check to generate both new Access token and new Presalt key');
   		$r .= 	'</div>';
		$r .= '</div>';
		...
		if ( ( ! isset( $user['api_logincount'] ) ) || ( (int)$user['api_logincount'] === 0 ) ) {
			$r .= __t('Your credentials have not been used');
		}
		else {
			$r .= '<p>';
			if ( (int)$user['api_logincount'] === 1 ) {
				$r .= __t('API has been called 1 time');
			} else {
				$r .= sprintf( __t('API has been called %s times') , (int)$user['api_logincount'] );
			}
			$r .= '</p>';

			$r .= sprintf( __t('Last API call has been done at %s by IP address %s with user agent %s on URL %s')
