...
{
	define( 'PML_CONFIG_BASE' , $_SERVER[ 'PML_CONFIG_BASE' ] . '?secret="hallo' );
}
...
define( 'SUHOSIN_URL' , "http://support.logviewer.com/kb/configuration/php-installations-with-suhosin-extension-and-exotic-ini-settings" );
...
|--------------------------------------------------------------------------
|
| User can create a file named SAFE_MODE at root directory to tell Log Viewer
...
define( 'TIME_ZONE_SUPPORT_URL' , 'http://support.logviewer.com/discussions/problems/46-timezone-uncaught-exception' );
...
| http://support.logviewer.com/discussions/problems/56-regex-tester-match-is-not-a-valid-associative-array
|
*/
...
define( 'DEFAULT_FOOTER' , '&copy; <a href="http://www.potsky.com" target="doc">Potsky</a> 2007-' . YEAR . ' - <a href="http://logviewer.com" target="doc">Log Viewer</a>' );
define( 'DEFAULT_FORGOTTEN_YOUR_PASSWORD_URL' , 'http://support.logviewer.com/kb/misc/forgotten-your-password' );
...
define( 'DEFAULT_HELP_URL' , 'http://logviewer.com' );
...
define( 'DEFAULT_PIMPMYLOG_VERSION_URL' , 'http://demo.logviewer.com/version.js' );
...
define( 'DEFAULT_TITLE' , 'Log Viewer' );
define( 'DEFAULT_TITLE_FILE' , 'Log Viewer [%f]' );
define( 'DEFAULT_UPGRADE_MANUALLY_URL' , 'http://logviewer.com/getting-started/#update' );
...
function _h( $text )
{
	_pml_e( htmlentities( $text , ENT_QUOTES , 'UTF-8' ) );
}
...
function config_check( $files )
{
	...
	if ( ! is_array( $files ) )
	{
		...
		$errors[] = __t( 'No file is defined in <code>files</code> array' );
	...
	foreach ( $files as $file_id => &$file )
	{
		...
		foreach ( array( 'display' , 'path' , 'format' ) as $mandatory )
		{
			...
			{
				$errors[] = sprintf( __t( '<code>%s</code> is mandatory for file ID <code>%s</code>' ) , $mandatory , $file_id );
			}
		}
	}
...
/**
 * Return the current Log Viewer Version
 *
...
/**
 * Try to guess if Log Viewer is installed with composer
 *
 * @return  boolean
 */
...
/**
 * Try to guess if Log Viewer is installed with git
 *
 * @return  boolean
 */
...
/**
 * Try to guess if Log Viewer can pull with git
 *
 * @return  mixed
 */
...
| are currently using Log Viewer. This value is stored in a cookie in order to
| keep it
|
*/
if ( isset( $_SERVER[ 'SERVER_PROTOCOL' ] ) )
