...
if ( ! isset( $files[$file_id] ) ) {
    $return['error'] = sprintf( __t( 'File ID <code>%s</code> does not exist, please review your configuration file and stop playing!' ) , $file_id );
    echo json_encode( $return );
    die();
}
...
if ( ! file_exists( $file_path ) ) {
    $return['error'] = sprintf( __t( 'You have not enabled WP Debug yet, so your log file is empty. <br><br>Open up wp-config.php and add the following to it.<br><br><code>define( "WP_DEBUG", true );<br>
define( "WP_DEBUG_DISPLAY", false );<br>
define( "WP_DEBUG_LOG", true );</code><br><br> Then come back to the log viewer!' ) , $file_id );
    echo json_encode( $return );
    die();
}
...
if ( is_array( $errors ) ) {
    $return['error'] = __t( 'Configuration file has changed and is buggy now. Please refresh the page.' );
    echo json_encode( $return );
    die();
}
...
if ($reset === 1) {
    ...
    if ($data_to_parse < 0) { // Log file has been rotated, read all. It is not possible on apache because server is restarted gracefully but perhaps user has done something...
        $data_to_parse    = $new_file_size;
        $full             = true;
        $return['notice'] = '<strong>'. $now . '</strong> : ' . sprintf( __t('Log file has been rotated (previous size was %s and new one is %s)') , human_filesize($old_file_size) , human_filesize($new_file_size) );
    }
    if ($old_file_size === 0) {
        $full = true;
    }
...
if ( ! is_array( $logs ) ) {
    switch ( $logs ) {
        case '1':
            $return['error'] = sprintf( __t( 'File <code>%s</code> for file ID <code>%s</code> does not exist anymore...' ) , $file_path , $file_id );
            break;

        default:
            $return['error'] = sprintf( __t( 'Unknown error %s' ) , $logs );
            break;
    }
}
...
*/
    ...
    if ( @$logs['notice'] === 1 ) {
        $return[ 'notice' ] = '<strong>'. $now . '</strong> &gt; ' . __t('Log file has been rotated');
    }
    ...
    */
    ...
    |
    */
    $return['footer']   = sprintf( __t( '%s in <code>%sms</code> with <code>%s</code> of logs, <code>%s</code> skipped line(s), <code>%s</code> unreadable line(s).<br/>File <code>%s</code> was last modified on <code>%s</code> at <code>%s</code>, size is <code>%s</code>%s' )
        , ( $load_more === false )
            ? (
                ( $ln > 1 )
                ? sprintf( __t('%s new logs found') , $ln )
                : (
                    ( $ln === 0 )
                        ? __t( 'no new log found')
                        : __t( '1 new log found')
                )
            )
            : (
                ( $ln > 1 )
                ? sprintf( __t('%s old logs found') , $ln )
                : (
                    ( $ln === 0 )
                        ? __t( 'no old log found')
                        : __t( '1 olg log found')
                )
            )
        , $return['duration']
        ...
        , ( isset( $files[ $file_id ][ 'format' ][ 'type' ] ) )
            ? ', ' . sprintf( __t('log type is <code>%s</code>') , $files[ $file_id ][ 'format' ][ 'type' ] )
            : ''
    );
